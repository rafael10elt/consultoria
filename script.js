...
function createKanbanCard(v){
    ...
    // INÍCIO DA CORREÇÃO: Função checkOverdueVisits que estava faltando
    async function checkOverdueVisits(){
        const t=new Date(); 
        t.setHours(0,0,0,0); 
        const oD=visitsData.filter(v=>v.Status==='Agendada'&&new Date(v.DataVisita+'T23:59:59')<t); 
        if(oD.length>0){
            showLoadingOverlay('Atualizando status de visitas vencidas...'); 
            await Promise.all(oD.map(v=>nocoFetch(`Visitas/${v.Id}`,{method:'PATCH',body:JSON.stringify({Status:'Remarcada'})}))); 
            await fetchVisitas(); 
            applyFilters(); 
            // Não chamar hideLoadingOverlay aqui, pois a função principal (fetchAllData) já faz isso.
        }
    }
    // FIM DA CORREÇÃO
